    ...
    private async Task PopulateQuotaSnapshotAsync(string token, CopilotUsageState state)
    {
        try
        {
            ...
            if (root.TryGetProperty("quota_snapshots", out var snapshots))
            {
                ...
                // In some Copilot environments, there's a smaller window often called 'spark' or just the first snapshot
                foreach (var prop in snapshots.EnumerateObject())
                {
                        ...
                        ent > 0)
                    {
                        ...
                        var remPct = UsageMath.CalculateRemainingPercent(ent - normRem, ent);
                        
                        details.Add(new ProviderUsageDetail
                        {
                            Name = $"{NormalizeSnapshotName(prop.Name)} quota",
                            Used = $"{remPct:F1}% remaining ({ent - normRem:F0}/{ent:F0} used)",
                            Description = "Short-term quota window",
                    ...
                    entitlement > 0)
                {
                    ...
                    details.Add(new ProviderUsageDetail
                    {
                        ...
                        Used = $"{state.Percentage:F1}% remaining ({used:F0}/{entitlement:F0} used)",
                        Description = "Main interactions quota",
                        DetailType = ProviderUsageDetailType.QuotaWindow,

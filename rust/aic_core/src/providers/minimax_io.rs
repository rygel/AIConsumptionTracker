...
/// MiniMax International (IO) provider
/// API endpoint: https://api.minimax.io/v1/user/usage
pub struct MinimaxIOProvider {
    client: Client,
}

impl MinimaxIOProvider {
...
#[async_trait]
impl ProviderService for MinimaxIOProvider {
    ...
    async fn get_usage(&self, config: &ProviderConfig) -> Vec<ProviderUsage> {
        if config.api_key.is_empty() {
            return vec![ProviderUsage {
                ...
                provider_name: "MiniMax (International)".to_string(),
                is_available: false,
        ...
        // Use custom base_url if provided, otherwise use international endpoint
        let url = config
            .base_url
            .clone()
            .unwrap_or_else(|| "https://api.minimax.io/v1/user/usage".to_string());
